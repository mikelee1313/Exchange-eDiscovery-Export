    ...
    try {
        ...
        if ($exportStatus.Status -eq "succeeded" -or 
            ...
            if (($exportStatus.DownloadUrls -and $exportStatus.DownloadUrls.Count -gt 0) -or
                ...
                Write-Host "Preparing to download $($downloadFiles.Count) file(s)..." -ForegroundColor Cyan
                Write-LogEntry -LogName $logPath -LogEntryText "Starting download of $($downloadFiles.Count) file(s) from export" -LogLevel "INFO"
                
                $downloadResults = @()
                $successCount = 0
                $failCount = 0
                
                for ($i = 0; $i -lt $downloadFiles.Count; $i++) {
                    $file = $downloadFiles[$i]
                    $fileNum = $i + 1
                    
                    # Handle both processed format (FileName, DownloadUrl, Size) and raw format (fileName, downloadUrl, size)
                    $fileName = if ($file.FileName) { $file.FileName } else { $file.fileName }
                    $downloadUrl = if ($file.DownloadUrl) { $file.DownloadUrl } else { $file.downloadUrl }
                    $fileSize = if ($file.Size) { $file.Size } else { $file.size }
                    
                    Write-Host "`n[$fileNum/$($downloadFiles.Count)] Downloading: $fileName (Size: $fileSize bytes)" -ForegroundColor Yellow
                    
                    # Create output path for this file
                    # If multiple files, append file number to avoid overwriting
                    if ($downloadFiles.Count -gt 1) {
                        $fileExtension = [System.IO.Path]::GetExtension($fileName)
                        $baseFileName = [System.IO.Path]::GetFileNameWithoutExtension($fileName)
                        $outputFilePath = "$OutputPath\${sanitizedName}_${fileNum}_${baseFileName}${fileExtension}"
                    }
                    else {
                        $fileExtension = [System.IO.Path]::GetExtension($fileName)
                    ...
                    Write-LogEntry -LogName $logPath -LogEntryText "Downloading file $fileNum of $($downloadFiles.Count): $fileName to $outputFilePath" -LogLevel "INFO"
                    
                    # Download the file using Save-M365ComplianceFile
                    try {
                        Write-Host "   Using Save-M365ComplianceFile for download..." -ForegroundColor Gray
                        ...
                        Write-LogEntry -LogName $logPath -LogEntryText "Failed to download file $fileName`: $_" -LogLevel "ERROR"
                ...
                Write-Host "  Total Files: $($downloadFiles.Count)" -ForegroundColor White
                Write-Host "  Successful: $successCount" -ForegroundColor Green
                Write-Host "  Failed: $failCount" -ForegroundColor $(if ($failCount -gt 0) { "Red" } else { "Gray" })
                Write-Host "=" * 60 -ForegroundColor Cyan
                
                Write-LogEntry -LogName $logPath -LogEntryText "Download completed: $successCount succeeded, $failCount failed out of $($downloadFiles.Count) total files" -LogLevel "INFO"
                ...
                return @{
                    Success = $false
                    Error   = "No download URLs found in export status"
                }
            }
            catch {
